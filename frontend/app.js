...
function renderNavBar() {
  const container = document.querySelector('#bottom-nav .nav-items');
  if (!container) return;

  const hasWorkouts = state.hasCustomPlan || state.activeDomains.includes('workouts');
  const hasBooks    = state.books.length > 0 || state.activeDomains.includes('books');
  const hasTasks    = state.tasks.length > 0 || state.activeDomains.includes('tasks');

  let html = `
    <button class="nav-item ${state.currentScreen === 'home' ? 'active' : ''}" data-screen="home" id="nav-home" aria-current="page">
  ...
  if (hasWorkouts) {
    html += `
      <button class="nav-item ${state.currentScreen === 'workouts' ? 'active' : ''}" data-screen="workouts" id="nav-workouts">
        <span class="nav-icon"><svg viewBox="0 0 24 24"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg></span>
        <span class="nav-label">אימונים</span>
      </button>`;
  }

  if (hasBooks) {
    html += `
      <button class="nav-item ${state.currentScreen === 'books' ? 'active' : ''}" data-screen="books" id="nav-books">
        ...
        <span class="nav-label">ספרים</span>
      </button>`;
  }

  if (hasTasks) {
    html += `
      <button class="nav-item ${state.currentScreen === 'tasks' ? 'active' : ''}" data-screen="tasks" id="nav-tasks">
  ...
  html += `
    <button class="nav-item ${state.currentScreen === 'logs' ? 'active' : ''}" data-screen="logs" id="nav-logs">
